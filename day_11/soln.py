...
monkeys_copy = [
    [
        data.copy() if type(data) == list else data
        for data in monkey
    ]
...
def process_round(monkeys, inspect_counts, relief, monkeys_modulo):
    for i, monkey in enumerate(monkeys):
        ...
        monkey, monkeys = process_monkey(monkey, 
                                         monkeys, 
                                         relief, 
                                         monkeys_modulo)
...
for _ in range(RELIEF_ROUNDS):
    monkeys, inspect_counts = process_round(monkeys,
                                            inspect_counts,
                                            True,
                                            monkeys_modulo)
...
print(f'Level of monkey business after {RELIEF_ROUNDS}'\
...
for _ in range(ZERO_RELIEF_ROUNDS):
    monkeys_copy, inspect_counts = process_round(monkeys_copy,
                                            inspect_counts,
                                            False,
                                            monkeys_modulo)
inspect_counts.sort(reverse=True)
...
print(f'Level of monkey business after {ZERO_RELIEF_ROUNDS}'\
